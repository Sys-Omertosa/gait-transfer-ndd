...
def loso_fit_all_folds_fixed(
    ...
    for train_idx, test_idx in outer_loso.split(X, y, groups):
        base = build_pipeline(clf_name, clf_template)
...
def evaluate_noise_sweep_within(
    ...
    for ci, clf_name in enumerate(CLF_ORDER):
        modal = within_results["classifiers"][clf_name]["modal_params"]
        fitted_folds = loso_fit_all_folds_fixed(X, y, groups, clf_name, modal)
...
def evaluate_noise_sweep_cross(
    ...
    X_target = target_pool.select(ALL_FEATURE_COLS).to_numpy().astype(np.float64)
...
def permutation_importance_within(
    ...
    for ci, clf_name in enumerate(CLF_ORDER):
        ...
        for j, fname in enumerate(ALL_FEATURE_COLS):
