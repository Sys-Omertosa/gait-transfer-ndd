...
% Local copy of figure PDFs in ./figures/ (keeps the deck portable)
...
\begin{frame}{Within-condition performance (summary)}
  \vspace{-0.3em}
  \begin{itemize}
    \item Best macro-$F_1$ (paper): PD DT $\approx 0.867$; HD XGBoost $\approx 0.956$; ALS KNN $\approx 0.830$.
    \item SMOTE vs.\ no-SMOTE pattern differs by condition and classifier (see heatmap).
  \end{itemize}
  \vspace{0.2em}
  \begin{center}
    \presfig[0.58]{f1_within_condition_heatmap.pdf}
  \end{center}
\end{frame}
...
\begin{frame}{Zero-shot transfer degradation}
  \vspace{-0.2em}
  \begin{center}
    \presfig[0.88]{degradation_heatmap.pdf}
  \end{center}
  \vspace{0.2em}
  \centering\footnotesize Transfer $\Delta F_1$ and directional asymmetry across disease pairs (stored analysis).
\end{frame}

\begin{frame}{Directional asymmetry of transfer}
  \begin{center}
    \presfig[0.90]{delta_f1_asymmetry_pairs.pdf}
  \end{center}
\end{frame}

\begin{frame}{SHAP importance shift ($\delta_j$)}
  \vspace{-0.2em}
  \begin{center}
    \presfig[0.80]{delta_j_heatmap.pdf}
  \end{center}
  \vspace{0.2em}
  \centering\footnotesize Dominant shift features (e.g., \texttt{cv\_swing}, \texttt{dfa\_alpha\_stride}) inform mechanistic interpretation.
\end{frame}

\begin{frame}{Uncertainty: CI width (stride vs.\ subject)}
  \begin{center}
    \presfig[0.78]{ci_width_comparison.pdf}
  \end{center}
  \vspace{0.2em}
  \centering\footnotesize Subject-level bootstrap intervals reflect realistic between-subject variability.
\end{frame}

\section{Robustness and unsupervised structure}

\begin{frame}{Noise robustness and PCA / clustering (overview)}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{fig6_noise_robustness.pdf}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{fig5_pca_kmeans.pdf}
    \end{column}
  \end{columns}
  \vspace{0.3em}
  \centering\footnotesize Controlled perturbations and unsupervised geometry complement the transfer benchmark.
\end{frame}
...
\begin{frame}{Limitations}
  \begin{itemize}
    ...
    \item \textbf{Disjoint control groups (A vs.\ B):} Group A is younger and faster on average than Group B (mean age $\sim$34 vs.\ $\sim$44 years; mean gait speed $\sim$1.43 vs.\ $\sim$1.28~m/s). Some apparent cross-condition degradation may reflect this control shift, not only disease-domain shift.
